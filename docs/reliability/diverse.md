...
should be, and what information it needs to contain. This is useful if we need the model to output a response inclusive of varied and detailed information. So even if they aren't completely perfect and overly 
simplistic, we can feed the model 25 "pretty good" zero-shot easy to generate open-ended prompts, instead instead of iteratively searching for the 1 perfect prompt that outputs everything needed for our use case.
